...
def create_gabor(n0, n1, level, sigmax, sigmay, angle, phase, lambda_, xp):
    ...
    val = xp.exp(-0.5 * ((x_theta / sigmax) ** 2 + (y_theta / sigmay) ** 2)) * xp.cos((x_theta * lambda_ / sigmax) + phase)
    psi = level * val / nn
...
def update_y(d1u0, d2u0, tmp1, tmp2, lambda1, lambda2, beta, xp):
    ...
    ng=xp.sqrt(xp.add(xp.square(t1),xp.square(t2))) + xp.finfo(float).eps # Adding a small epsilon to avoid division by zero
    mask = ng > 1.0 / beta
...
def VSNR_2D_FIJI_GPU(filters, u0, n0, n1, nit, beta, max, xp):
    """
...
def create_filters(filters, gu0, n0, n1, xp):
    """
    Create filters based on filters parameters stored in the filters array. The calculated filters are stored in the gpsi array.
    """
    i=0
    ...
    fd1=xp.fft.fft2(d1.reshape(n0,n1)).flatten() #TODO is it necessary to speicfy the plan ?? cufftPlan2d
    ...
    for filt in filters:
        ...
        ftmp=xp.multiply(xp.abs(fd1),xp.abs(fpsitemp)) # //compute product: ftmp = |fd1|*|fpsitemp|;)
        imax = xp.argmax(xp.abs(ftmp)) # find the index of the maximum element # cublasIsamax(handle, m, ftmp, 1, &imax);
        max1=ftmp[imax]
...
def vsnr_admm(u0, psi, n0, n1, nit, beta, xp):
    ...
    fu0=xp.fft.fft2(u0.reshape(n0,n1)).flatten() # TODO fftn ??
    ...
    d1u0=xp.real(xp.fft.ifft2(ftmp1.reshape(n0,n1),norm="forward").flatten()) # TODO is this correct xp.real ? #should be float but is complex
    d1u0=xp.divide(d1u0,n) # d1u0=normalize_array(d1u0)
    
    # // Computes d2u0
    ftmp2=xp.multiply(fd2,fu0)
    d2u0=xp.real(xp.fft.ifft2(ftmp2.reshape(n0,n1),norm="forward").flatten()) #TODO xp.fft.ifftn or xp.fft.ifft ??
    ...
    for k in range (nit):
        ...
        tmp1=xp.fft.ifft2(ftmp1.reshape(n0,n1),norm="forward").flatten().real #TODO ifft or ifftn ??
    ...
    u=xp.fft.ifft2(ftmp1.reshape(n0,n1),norm="forward").flatten().real #TODO ifft or ifftn ??
    u=xp.divide(u,n)
    u=xp.subtract(u0,u)
    
    return u

def vsnr2d(img, filters, nite=20, beta=10., xp=np):
    r"""
    ...
    beta: float
        The regularization parameter in the VSNR model. Controls the trade-off between the data fidelity term and the regularization term. A higher beta value gives more weight to the regularization term, which encourages sparser solutions but may result in more denoising (and potentially more distortion). A lower beta value gives more weight to the data fidelity term, which encourages solutions that are closer to the original image but may result in less denoising.
    ...
    # calculation
    vmax = u0.max()
    u=VSNR_2D_FIJI_GPU(filters, u0, n0, n1, nite, beta, vmax, xp)
