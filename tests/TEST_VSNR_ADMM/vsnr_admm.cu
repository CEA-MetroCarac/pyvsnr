...
__global__ void compute_phi(CuC* fphi1, CuC* fphi2, CuC* fphi, float beta, int n)
{
    ...
    // cudaMemcpy(test, fphi, n * sizeof(float), cudaMemcpyDeviceToHost);
...
void VSNR_ADMM_GPU(float* u0, float* psi, int n0, int n1, int nit, float beta, float* u, int dimGrid, int dimBlock)
{
    ...
    float *test = new float[n];   
        cudaMemcpy(test,psi, n * sizeof(float), cudaMemcpyDeviceToHost);
        for (int i = 0; i < 10; i++) { 
            printf(" psi: %f\n", test[i]); 
    }
    ...
    // float * u3;
    // cudaMalloc((void**)&u3, m*sizeof(CuC));
    
    cufftExecC2R(planC2R, ftmp1, u);
    // Allocate host memory for the copy
    
    // float *test1 = new float[n];
    // // Copy the data from the GPU to the host
    // cudaMemcpy(test1, u0, n * sizeof(float), cudaMemcpyDeviceToHost);
    // // Now you can print the values
    // for (int i = 0; i < 10; i++) { 
    //     printf(" u0 after C2R: %f\n", test1[i]); 
    // }
...
int main( void ) {
    ...
    // Then, before your loop that uses rand(), add this line:
    srand(time(0));
    ...
    float* u0;
    cudaMalloc(&u0, data1.size() * sizeof(float)); // u0=&data1[0];
    cudaMemcpy(u0, data1.data(), data1.size() * sizeof(float), cudaMemcpyHostToDevice);


    // Allocate host memory for the copy
    float* u0_host = new float[data1.size()];

    // Copy the data from the GPU to the host
    cudaMemcpy(u0_host, u0, data1.size() * sizeof(float), cudaMemcpyDeviceToHost);
    ...
    // // Allocate host memory for the copy
    // float* psi_host = new float[data.size()];

    // // Copy the data from the GPU to the host
    // cudaMemcpy(psi_host, psi, data.size() * sizeof(float), cudaMemcpyDeviceToHost);

    FILE *fp = fopen("u0.txt", "w"); 
    if (fp == NULL) { 
    ...
    cudaMemcpy(gpsi_h, gpsi, m*sizeof(float), cudaMemcpyDeviceToHost);
    
    float* d_u;
    cudaMalloc(&d_u, n0 * n1 * sizeof(float));
    cudaMemcpy(d_u, u, n0 * n1 * sizeof(float), cudaMemcpyHostToDevice);

    VSNR_ADMM_GPU(u0, gpsi, n0, n1, nit, beta, d_u, dimGrid, dimBlock);

    // Allocate host memory for the copy
    float* u_host = new float[512*512];

    // Copy the data from the GPU to the host
    cudaMemcpy(u_host, d_u, n * sizeof(float), cudaMemcpyDeviceToHost);
